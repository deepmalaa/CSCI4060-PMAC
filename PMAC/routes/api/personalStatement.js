...
const uploadMiddleware = (req, res, next) => {
  const upload = store.single('personalstatement');
...
router.post('/upload', auth, uploadMiddleware, async (req, res) => {
  ...
  //check if user already has a personalstatement
  let currentPic = foundUser.personal_statement;
  ...
  Profile.findByIdAndUpdate(profileId ,{personal_statement: id}, {userFindAndModify: true, new:true})
  .then(profile => res.send(profile.personal_statement))
  .catch((err) =>{
    console.log('db error in updating personal_statement', err)
  });
});
